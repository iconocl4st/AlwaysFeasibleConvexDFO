...
def test_for_improvement(state, model, tr_solution):
	...
	assert expected_decrease >= 0, 'did not expect decrease: ' + str(expected_decrease)
	...
	if tr_solution['gc-value'] is not None:
		...
		assert projected_gradient_decrease >= -1e-8, 'generalized cauchy did not find decrease'
	else:
		projected_gradient_decrease = None

	state.logger.verbose_json('checking if decrease expected', {
	...
	if distance_to_trial < 0.1 and (
			projected_gradient_decrease is None or expected_decrease < 0.1 * projected_gradient_decrease):
	...
	assert np.abs((current_value - actual_value)) / max(abs(current_value), 1) < 1e-4, \
	...
	tr_active = np.max(np.abs(tr_solution['trial-point'] - state.current_iterate)) > 0.9 * state.outer_tr_radius
